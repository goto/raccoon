	...
	for i := 0; i < w.Size; i++ {
		go func(workerName string) {
			...
			for request := range w.EventsChannel {
				...
				// Apply policy filtering before publishing to Kafka.
				eventsToPublish := w.applyPolicy(request.GetEvents(), request.ConnectionIdentifier.Group)

				err := w.kafkaProducer.ProduceBulk(eventsToPublish, request.ConnectionIdentifier.Group, deliveryChan)
